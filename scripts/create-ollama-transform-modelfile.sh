...
# Create Ollama Custom Model from Transform Modelfile
# This script creates a custom model with a system prompt for description transformation

set -e

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
MODELLFILE="$PROJECT_ROOT/config/ollama-transform-modelfile"
MODEL_NAME="anthroposcenic-transform:latest"

echo "📝 Creating Ollama Transform Model from Modelfile"
echo ""
...
# Create model from modelfile (use --force to update if exists)
if ollama create "$MODEL_NAME" -f "$MODELLFILE" --force 2>/dev/null || ollama create "$MODEL_NAME" -f "$MODELLFILE"; then
    echo ""
    echo "✅ Custom model '$MODEL_NAME' created successfully!"
    echo ""
    echo "You can now use this model for description transformation."
    echo "It includes a system prompt optimized for scientific analogies."
    echo ""
    echo "To verify, run:"
